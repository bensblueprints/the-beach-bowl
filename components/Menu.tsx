...
const ACCENT: Record<
  MenuCategory["accent"],
  { chip: string; dot: string; card: string; ring: string }
> = {
  acai: {
  ...
  return (
    <section className="relative bg-sand-100 py-24">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mt-24 rounded-3xl border border-ink/10 bg-white p-8 shadow-sm md:p-10">
          <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-ink/60">
                Add-on
              </p>
              <h3 className="mt-2 font-display text-3xl">Lotus Energy Drink</h3>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {cat.items.map((item, i) => (
          <motion.a
            ...
            className={`group relative flex flex-col justify-between rounded-2xl bg-gradient-to-br p-6 ring-1 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_22px_40px_-18px_rgba(26,22,37,0.2)] ${a.card} ${a.ring}`}
          >
            <div>
              <div className="flex items-start justify-between gap-3">
                ...
                {item.tag && (
                  <span
                    className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${a.chip}`}
                  >
                    {item.tag}
                  </span>
                )}
